...
## Ticket validation migration

If you see **Unknown column 'tord.validated_at'** when using the Validate tab (event ticket orders), add the column by running the migration once:

1. **Deploy** (so the migration Lambda is available):
   ```bash
   cd backend && sam build && sam deploy
   ```

2. **Call the migration in AWS** (from `backend/`, using the same profile/region as your deploy):
   ```bash
   cd backend && ./run-migrate-ticket-validated.sh
   ```
   Or manually (replace with your API base URL):
   ```bash
   curl -X POST "https://YOUR_API_ID.execute-api.eu-west-1.amazonaws.com/Prod/admin/migrate-add-ticket-validated"
   ```
   The route uses `Auth: NONE`, so no token is required. On success you get:
   `{"success":true,"message":"ticket_orders.validated_at migration completed"}`

After that, the Validate tab and ticket-orders API will work.

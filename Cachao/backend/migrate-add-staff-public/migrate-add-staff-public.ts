...
export const lambdaHandler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  try {
    ...
    if (count === 0) {
      // Add is_public column (default to false for existing records, true for new artists)
      await connection.query(`
        ALTER TABLE event_staff
        ADD COLUMN is_public BOOLEAN DEFAULT FALSE AFTER role
      `);
      console.log('Column is_public added successfully to event_staff table');
    } else {
      console.log('Column is_public already exists in event_staff table');
    }
    ...
    return {
      ...
      body: JSON.stringify({
        success: true,
        message: 'Staff is_public column migration executed successfully',
      }),
    };
  } catch (error) {
